...
public class Unstuck implements DiscordCommandExecutor {
    ...
    public void execute(GuildMember source, List<String> args, MessageReceivedEvent event) throws DiscordCommandException {
        ...
        String playerUUID = target.map(User::getName).orElseThrow(()->new DiscordCommandException("Could not locate players UUID."));
        Sponge.getCommandManager().process(Sponge.getServer().getConsole(), String.format("spawn other %s", playerUUID));
    }
}
